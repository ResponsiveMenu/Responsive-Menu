...
document.addEventListener('DOMContentLoaded', () => {
  ...
  function addSubmenuLevelClasses(parentEl, level) {
    ...
    for (let i = 0; i < children.length; i++) {
      ...
      if (child.classList.contains('wp-block-navigation-submenu') || child.classList.contains('wp-block-navigation__submenu-container')) {
        const existingLevelClass = Array.from(child.classList).find(cls => cls.startsWith('rmp-block-submenu-level-'));
  ...
  parentElements.forEach(parent => {
    ...
    if (submenuIconType === 'icon') {
      const inactiveEl = menuContainer ? menuContainer.querySelector('.rmp-submenu-trigger-icon .rmp-inactive-submenu-trigger-icon') : null;
      const activeEl = menuContainer ? menuContainer.querySelector('.rmp-submenu-trigger-icon .rmp-active-submenu-trigger-icon') : null;
      ...
      subArrow.addEventListener('click', event => {
        ...
        if (isActive) {
          ...
          if (menuParent.getAttribute('data-auto-expand-parent') === 'true') {
            const childArrows = parent.querySelectorAll(':scope > .wp-block-navigation__submenu-container .wp-block-navigation-submenu:not(.rmp-block-active-submenu) > .rmp-block-menu-subarrow');
          ...
          if (menuParent.getAttribute('data-use-accordion') === 'true') {
            ...
            if (parentContainer) {
              const siblings = parentContainer.querySelectorAll(':scope > .wp-block-navigation-submenu.rmp-block-active-submenu');
      ...
      openSubmenus.forEach(item => {
        ...
        if (arrow) {
          ...
          if (iconType === 'image' && inactiveIcon) {
            ...
            const inactiveEl = container ? container.querySelector('.rmp-submenu-trigger-icon .rmp-inactive-submenu-trigger-icon') : null;
